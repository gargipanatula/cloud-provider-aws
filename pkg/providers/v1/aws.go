...
func (c *Cloud) buildNLBHealthCheckConfiguration(svc *v1.Service) (healthCheckConfig, error) {
	hc := healthCheckConfig{
		...
		Interval:           int32(defaultNlbHealthCheckInterval),
		Timeout:            int32(defaultNlbHealthCheckTimeout),
		HealthyThreshold:   int32(defaultNlbHealthCheckThreshold),
		UnhealthyThreshold: int32(defaultNlbHealthCheckThreshold),
	}
...
func (c *Cloud) EnsureLoadBalancer(ctx context.Context, clusterName string, apiService *v1.Service, nodes []*v1.Node) (*v1.LoadBalancerStatus, error) {
	...
	for _, port := range apiService.Spec.Ports {
		...
		if isNLB(annotations) {
			...
			if port.Protocol != v1.ProtocolUDP && certificateARN != "" && (sslPorts == nil || sslPorts.numbers.Has(int32(port.Port)) || sslPorts.names.Has(port.Name)) {
	...
	if accessLogEmitIntervalAnnotation != "" {
		accessLogEmitInterval64, err := strconv.ParseInt(accessLogEmitIntervalAnnotation, 10, 32)
		accessLogEmitInterval32 := int32(accessLogEmitInterval64)
		if err != nil {
		...
		loadBalancerAttributes.AccessLog.EmitInterval = &accessLogEmitInterval32
	}
	...
	if connectionDrainingTimeoutAnnotation != "" {
		connectionDrainingTimeout64, err := strconv.ParseInt(connectionDrainingTimeoutAnnotation, 10, 32)
		connectionDrainingTimeout32 := int32(connectionDrainingTimeout64)
		if err != nil {
			return nil, fmt.Errorf("error parsing service annotation: %s=%s",
		...
		loadBalancerAttributes.ConnectionDraining.Timeout = &connectionDrainingTimeout32
	}
	...
	if connectionIdleTimeoutAnnotation != "" {
		connectionIdleTimeout64, err := strconv.ParseInt(connectionIdleTimeoutAnnotation, 10, 32)
		connectionIdleTimeout32 := int32(connectionIdleTimeout64)
		if err != nil {
			return nil, fmt.Errorf("error parsing service annotation: %s=%s",
		...
		loadBalancerAttributes.ConnectionSettings.IdleTimeout = &connectionIdleTimeout32
	}
